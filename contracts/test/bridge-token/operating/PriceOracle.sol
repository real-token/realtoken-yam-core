...
contract PriceOracle is Initializable, IPriceOracle, Operator {
  ...
  */
    ...
    external override onlyOperator 
  {
    ...
    for (uint256 i = 0; i < _currency1.length; i++) {
      ...
      _prices[_currency1[i]][_currency2[i]] = Price(_price[i], _decimals[i], block.timestamp);
      // solium-disable-next-line security/no-block-members
      emit PriceSet(_currency1[i], _currency2[i], _price[i], _decimals[i], block.timestamp);
    }
  }
  ...
  */
  function setPrice(bytes32 _currency1, bytes32 _currency2, uint256 _price, uint8 _decimals) public override onlyOperator {
    // solium-disable-next-line security/no-block-members
    _prices[_currency1][_currency2] = Price(_price, _decimals, block.timestamp);
    // solium-disable-next-line security/no-block-members
    emit PriceSet(_currency1, _currency2, _price, _decimals, block.timestamp);
  }
